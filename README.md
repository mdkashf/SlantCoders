...
In our driver class, we experimented with the size of the matrix, going from 500 up to 10,000 (due to memory constraints, we did not test greater than 10,000) by increments of 500. We randomly generated a number, and searched for this number in the randomly generated matrix a number of times, through our for-loop. The time in milliseconds was recorded right before find(matrix, number) was called, and also recorded right after the method was called. The difference was added to a sum, which was then divided by the number of trials, so as to find the average run time. 
...
As the graphs indicate, r^2 for the trendlines is very close to 1.
